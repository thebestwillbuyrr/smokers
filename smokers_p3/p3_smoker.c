...
int main(int argc, char **argv) {
    ...
    // подключаемся к shared memory агента
    int shm_fd = shm_open(SHM_NAME_P3, O_RDWR, 0);
    if (shm_fd == -1) {
    ...
    g_mutex = sem_open(SEM_MUTEX_P3, 0);
    g_agent_sem = sem_open(SEM_AGENT_P3, 0);
    g_sm0 = sem_open(SEM_SM0_P3, 0);
    g_sm1 = sem_open(SEM_SM1_P3, 0);
    g_sm2 = sem_open(SEM_SM2_P3, 0);
    ...
    if (log_fd == -1) {
        fprintf(stderr, "[smoker %d] не могу открыть FIFO (наблюдатель не запущен?)\n", g_id);
    } else {
